...
%         disp(['get err = ' num2str(err)])
        ...
        input_array(1,run+1) = plant_input;
        if run == 1 
            input_array(1,run) = plant_input;
        end
        if run < 240
            if command(1,run) == command(1,run+1)
                y_result = Plant(input_array(1,run+1));
            else
                y_result = Plant(input_array(1,run));
            end
        else
            y_result = Plant(input_array(1,run));
        end
    ...
    for i = 1:pop_size
        for j = 1:DNA_size 
            ...
            if j == 1
                ...
                Init(i, j) = Init(i, j)*0.002;
            end
            Init(i, j) = sort(Init(i, j));
        end
    end
end
